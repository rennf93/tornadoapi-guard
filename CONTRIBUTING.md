Contributing to TornadoAPI Guard
=================================

Thank you for considering contributing to TornadoAPI Guard! This document outlines the process for contributing to this Python library and helps ensure a smooth collaboration experience.
...
- Specify your environment: OS, Python version, Tornado version, etc.
...
- Explain why this enhancement would be useful to TornadoAPI Guard users
...
The documentation for TornadoAPI Guard is built with MkDocs. To build and view the documentation locally:
...
TornadoAPI Guard follows the same adapter pattern as the rest of the Guard ecosystem. The key integration point is Tornado's `RequestHandler.prepare()` hook (async) for the security pipeline, and `on_finish()` for response processing.

Since Tornado is async, this adapter uses the async `guard-core` (not `guard_core.sync`), matching FastAPI Guard's approach.
...
Thank you for contributing to TornadoAPI Guard!
