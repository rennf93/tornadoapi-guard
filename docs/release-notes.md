...
- **Thin adapter over guard-core**: TornadoAPI Guard is the Tornado binding for the [guard-core](https://github.com/rennf93/guard-core) framework-agnostic security engine.
- **Tornado-native integration**: `SecurityHandler` base class that integrates with Tornado's `RequestHandler.prepare()` / `on_finish()` lifecycle.
- **Full feature parity with fastapi-guard**: IP control, rate limiting, penetration detection, behavioral analysis, security headers, Redis integration, and all decorators from guard-core.
- **Full branch coverage**: 100% statement and branch coverage across the adapter, middleware, and handler layers.
...
- **Guard-Core migration**: TornadoAPI Guard is now a thin adapter over [guard-core](https://github.com/rennf93/guard-core), the framework-agnostic security engine. All security logic (17 checks, 8 handlers, detection engine) lives in guard-core; this package provides only the Tornado integration layer.
- **Zero breaking changes to public API**: All existing imports (`from tornadoapi_guard import SecurityConfig`, `from tornadoapi_guard import SecurityMiddleware`, etc.) continue to work exactly as before.
...
- **Namespace Consistency**: All TornadoAPI Guard components now use consistent `tornadoapi_guard.*` logger namespace hierarchy
  - Root logger: `tornadoapi_guard`
  - Handlers: `tornadoapi_guard.handlers.{component}`
  - Decorators: `tornadoapi_guard.decorators.{component}`
  - Detection Engine: `tornadoapi_guard.detection_engine`
...
- **Logger Isolation**: TornadoAPI Guard logs are now properly isolated from user application logs
...
- Fixed IPv6 address support throughout the project - PR [#51](https://github.com/rennf93/tornadoapi-guard/pull/51) - Issue [#50](https://github.com/rennf93/tornadoapi-guard/issues/50)
...
- Created an interactive [TornadoAPI Guard Playground](https://playground.tornadoapi-guard.com)
...
- Fixed Azure IP ranges download by adding proper User-Agent headers ([#19](https://github.com/rennf93/tornadoapi-guard/pull/19))
