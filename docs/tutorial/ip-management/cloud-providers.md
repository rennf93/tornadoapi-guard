...
title: Cloud Provider IP Blocking - TornadoAPI Guard
description: Block requests from major cloud providers like AWS, GCP, and Azure using TornadoAPI Guard's IP management
...
TornadoAPI Guard can automatically detect and block requests from major cloud providers. The IP ranges for these providers are only loaded when cloud blocking is enabled, improving startup performance.
...
```python
from tornadoapi_guard import cloud_handler
...
```python
from tornadoapi_guard import SecurityHandler, cloud_handler


class CheckCloudHandler(SecurityHandler):
    async def get(self, ip: str) -> None:
        is_cloud = cloud_handler.is_cloud_ip(
            ip,
            providers={"AWS", "GCP", "Azure"},
        )
        self.write({"ip": ip, "is_cloud": is_cloud})
```

Register as `(r"/check-cloud/([^/]+)", CheckCloudHandler)` in your `tornado.web.Application`.
