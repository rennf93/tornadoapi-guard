...
title: IP Geolocation - TornadoAPI Guard
description: Configure country-based IP filtering and geolocation features using IPInfo's database in TornadoAPI Guard
...
TornadoAPI Guard accepts an arbitrary class that implements geolocation and country-based filtering. All it needs is to implement the following protocol:
...
```python
from tornadoapi_guard import IPInfoManager, SecurityHandler

ipinfo_db = IPInfoManager(token="your_ipinfo_token_here")
await ipinfo_db.initialize()


class CountryLookupHandler(SecurityHandler):
    async def get(self, ip: str) -> None:
        country = ipinfo_db.get_country(ip)
        self.write({"ip": ip, "country": country})
```

Register the handler as `(r"/country/([^/]+)", CountryLookupHandler)` in your `tornado.web.Application`.
