...
title: Penetration Detection - TornadoAPI Guard
...
TornadoAPI Guard includes sophisticated penetration attempt detection to identify and block malicious requests.
...
```python
import json
from tornadoapi_guard import SecurityHandler, sus_patterns_handler


class SecureEndpointHandler(SecurityHandler):
    async def post(self) -> None:
        ...
        if result["is_threat"] and result["threat_score"] > 0.8:
            ...
            self.set_status(403)
            self.write({"error": f"Threat detected: {', '.join(threat_types)}"})
            return
...
When `passive_mode` is enabled, TornadoAPI Guard will:
...
Integration with TornadoAPI Guard Agent
